<template>
  <header class="header">
    ...
    <div class="navbar">
      <div class="navbar-left-container">
        <a href="/">
          ...
          <h2 style="padding-left: 10px;color: #d1434a;">滨仔商城</h2>
        </a>
      </div>
      <div class="navbar-right-container" style="display: flex;">
        <div class="navbar-menu-container">
          <span class="navbar-link" v-show="nickName">{{nickName}}，欢迎你</span>
          <a href="javascript:void(0)" class="navbar-link" @click="loginModelFlag=true" v-show="!nickName">Login</a>
    ...
    <div class="md-modal modal-msg md-modal-transition" :class="{'md-show': loginModelFlag}">
      <div class="md-modal-inner">
        <div class="md-top">
          ...
          <button class="md-close" @click="hideLoginBox">Close</button>
        </div>
          ...
          <div class="confirm-tips">
            ...
            <ul>
              <li class="regi_form_input">
                ...
                <input type="text" tabindex="1" name="loginname" class="regi_login_input regi_login_input_left"
                       placeholder="User Name" data-type="loginname" v-model="userName">
              </li>
    ...
    <div class="md-overlay" v-show="loginModelFlag" @click="hideLoginBox"></div>
  </header>
</template>
  ...
  export default {
    data() {
      return {
        userName: "",
        userPwd: "",
        errorTip: false, // 控制错误显示
        loginModelFlag: false, // 控制遮罩和Login显示
      }
    },
    computed: {
    ...
    methods: {
      ...
      getCartCount() {
        axios.get('/users/cartCount').then(result => {
          let res = result.data;
          if (res.status == 0) {
            this.$store.commit('setCartCount', res.result.cartCount);
          }
        })
      }
    }
