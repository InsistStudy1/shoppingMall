<template>
  <div>
    ...
    <div class="container">
              ...
              <li v-for="(item, index) in cartList" :key="index">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" @click="editCartGood('check', item)"
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="editCartGood('reduce', item)">-</a>
                        <span class="select-ipt">{{ item.productNum }}</span>
                        <a class="input-add" @click="editCartGood('add', item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="showDelConfirm(item.productId, item.productNum)">
      ...
      <Modal :modalShow="modalShow" @hideModal="hideModal">
        <p slot="message">你确认要删除该商品</p>
        <div slot="btnGroup">
          <a href="javascript:;" class="btn btn--m" @click="delCartGood">确认</a>
          <a href="javascript:;" class="btn btn--m" @click="modalShow=false">取消</a>
        </div>
      </Modal>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      checkAllFlag() {
        if (this.checkedCount == this.cartList.length) return true;
        return false;
      },
      checkedCount() {
        let count = 0;
    ...
    methods: {
      getCartList() {
        axios.get('/users/cartList').then(result => {
          let res = result.data;
          if (res.status == 0) {
            return this.cartList = res.result.cartList
          }
        })
      },
      showDelConfirm(productId, productNum) {
        this.modalShow = true;
      ...
      delCartGood() {
        axios.post('/users/deleteCartGood', {productId: this.productId}).then(result => {
          let res = result.data;
          if (res.status == 0) {
            this.modalShow = false;
      ...
      editCartGood(flag, item) {
        ...
        if (flag == 'reduce') {
          if (item.productNum == 1) {
            return this.showDelConfirm(item.productId);
          }
          item.productNum--;
          editNum--;
        }
        ...
        axios.post('/users/editCartGood', {
          productId: item.productId,
          productNum: item.productNum,
          checked: item.checked
        }).then(result => {
          if (result.data.status == 0) {
            this.$store.commit('updateCartCount', editNum);
          } else {
      ...
      selectAll() {
        ...
        axios.post('/users/editCheckAll', {checkAllFlag: flag}).then(result => {
          if (result.data.status == 0) {
          } else {
            console.log(result.data.message)
          }
