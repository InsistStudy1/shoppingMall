      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
              ...
              <ul>
                <li v-for="(item, index) in addressListFilter" :key="index" :class="{'check': checkIndex==index}" @click="checkIndex=index">
                  ...
                  <div class="addr-opration addr-set-default">
                    <a href="javascript:;" class="addr-set-default-btn" v-show="!item.isDefault" @click="setDefault(item.addressId)"><i>Set default</i></a>
                  </div>
            ...
            <div class="shipping-addr-more" v-show="moreFlag">
              <a class="addr-more-btn up-down-btn" href="javascript:;" :class="{'open': limit>3}" @click="more">
                more
                <i class="i-up-down">
          ...
          <div class="next-btn-wrap">
            <router-link :to="{path: 'orderconfirm', query: {'addressId': defaultAddressId}}" class="btn btn--m btn--red">Next</router-link>
          </div>
          <Modal :modalShow="modalShow" @hideModal="hideModal">
            <p solt="message">你确定要删除该地址吗</p>
...
<script>
  ...
  export default {
    ...
    methods: {
      getAddressList() {
        axios.get('/users/addressList').then(result => {
          ...
          if (res.status == 0) {
            ...
            // 判断是否显示更多
            if (this.addressList.length>3) return this.moreFlag = true;
            this.moreFlag = false
          }
        })
      },
      more() {
        if (this.limit == 3) {
          return this.limit = this.addressList.length;
        }
        this.limit = 3;
      },
      setDefault(addressId) {
        axios.post('users/setDefault', {addressId}).then(result => {
          if (result.data.status == 0) {
            this.getAddressList();
          }
        })
      },
      delAddressConfirm(delAddressId) {
        this.modalShow = true;
      ...
      delAddress() {
        axios.post('/users/delAddress', {addressId: this.delAddressId}).then(result => {
          let res = result.data;
          if (res.status == 0) {
            this.modalShow = false;
            this.getAddressList();
